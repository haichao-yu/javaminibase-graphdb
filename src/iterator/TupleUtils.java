...
public class TupleUtils {
     ...
     */
    public static double CompareTupleWithTuple(AttrType fldType, // yhc: change from int to double
    ...
    public static double CompareTupleWithTuple(AttrType fldType, // yhc: change from int to double
        ...
        if (fldType.attrType == AttrType.attrDesc) {

            Descriptor t1_d = t1.getDescFld(t1_fld_no);
            Descriptor t2_d = t2.getDescFld(t2_fld_no);

            if (target.equals(new Descriptor(-1, -1, -1, -1, -1))) {
                ...
                if (t1_d.distance(target) == t2_d.distance(target)) {
                    return 0;
                }
                else if (t1_d.distance(target) < t2_d.distance(target)) {
                    return 1;
                }
                else {
                    return -1;
     ...
     * @throws TupleUtilsException exception from this class
     */
    public static double CompareTupleWithValue(AttrType fldType, // yhc: change from int to double
                                            Tuple t1, int t1_fld_no,
                                            Tuple value)
            throws IOException,
            UnknowAttrType,
